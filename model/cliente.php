<?php
    class Cliente{
        ...
        public function cadastrarCliente($pdo){
            $sth = $pdo->prepare("INSERT INTO clientes (nome, email, cnpj, telefone, rua, bairro, numero, cep, cidade, estado, pais) VALUES (:nome,:email,:cnpj,:telefone,:rua,:bairro,:num,:cep,:cidade,:estado,:pais)");
        ...
        public function mostrarCliente($pdo){
            $sth = $pdo->query("SELECT * FROM clientes");
            $sth -> execute();
            return $users = $sth->fetchAll(PDO::FETCH_ASSOC);
        }

        static function pesquisarCliente($pdo, $id){
            $sth = $pdo->prepare("SELECT * from clientes WHERE idCliente=:id LIMIT 1");
            $sth->BindValue(':id', $id);
            $sth->execute();
        ...
        public function editarCliente($pdo,$id){
            $sth = $pdo->prepare("UPDATE clientes SET nome=:nome, email=:email, cnpj=:cnpj,  telefone=:telefone, rua=:rua, bairro=:bairro, numero=:num, cep=:cep, cidade=:cidade, estado=:estado, pais=:pais WHERE idCliente=:id");
            $sth->BindValue(':id', $id);
            $sth->BindValue(':nome', $this->nome);
            $sth->BindValue(':email', $this->email);
            $sth->BindValue(':cnpj', $this->cnpj);
        ...
        static function deletarCliente($pdo, $id){
            $sth = $pdo->prepare("DELETE from clientes WHERE idCliente=:id LIMIT 1");
            $sth->BindValue(':id', $id);
            return $sth->execute();
        }

        static function mostrarQuantidade($pdo){
            $sth = $pdo->prepare("SELECT COUNT(*) AS 'quantidade' FROM clientes");
            $sth->execute();
